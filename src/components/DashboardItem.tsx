        ...
        <Tooltip.Root>
          <Tooltip.Trigger asChild>
            <button
              ...
              className="absolute -top-2 -right-2 w-7 h-7 bg-red-500 text-white rounded-full text-base opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-red-600 flex items-center justify-center z-10 shadow-md font-medium"
            >
              ×
            </button>
          </Tooltip.Trigger>
          ...
          >
            <Image
              ...
              onError={(e) => {
                ...
                if (parent) {
                  parent.innerHTML = `<div class="text-sm leading-1 flex h-full w-full items-center justify-center bg-gray-700 text-white">${item.name.charAt(0)}</div>`;
                }
              }}
            />
          </div>
